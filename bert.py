...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    #normalize the scores using softmax along the last dimension
    attention_probs = F.softmax(scores, dim=-1)
    
    #apply dropout to attention probabilities
    attention_probs = self.dropout(attention_probs)
    
    #multiply attention probabilities with value to get the weighted sum
    context_layer = torch.matmul(attention_probs, value)
    
    #concatenate attention from all heads
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    normalized_output = ln_layer(transformed_output + input)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    self_output = self.add_norm(hidden_states, self_output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    
    #feed-forward layer
    intermediate_output = self.interm_af(self.interm_dense(self_output))
    
    #add-norm after feed-forward
    layer_output = self.add_norm(self_output, intermediate_output, self.out_dense, self.out_dropout, self.out_layer_norm)
