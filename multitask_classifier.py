...
class MultitaskBERT(nn.Module):
    ...
    # Compute dot product of the two embeddings
        dot_product = torch.sum(embeddings_1 * embeddings_2, dim=-1)
    
    # Compute the L2 norm (Euclidean length) of each embedding
        norm_1 = torch.norm(embeddings_1, dim=-1)
        norm_2 = torch.norm(embeddings_2, dim=-1)
...
# Run for the specified number of epochs
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(tqdm(sst_train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
            ...
            # Inside the training loop for the STS task
